...
| Administrator SSH/signing-key directory | `/home/root/.ssh/keys` |
...
The rules directory must be readable by the polkit service. The token directory must be root-owned and mode `0755`; token files are mode `0644` so `polkitd` can read them. The log directory should be root-owned and mode `0755`, and the log file root-owned mode `0640`. The public key is stored in the config as:
...
```bash
ssh-keygen -y -f "/home/root/.ssh/keys/admin@e3da.local" \
	| ssh -p 10022 -i "/home/root/.ssh/keys/admin@e3da.local" root@e3da-srv2.e3da.wx \
	"install -d -o yrpeng -g root -m 0755 /home/root/e3da/pkauth/{bin,conf}; printf 'ADMIN_PUBLIC_KEY=\\\"%s\\\"\\n' \"\$(cat)\" > /home/root/e3da/pkauth/conf/pkauth.conf; chown yrpeng:root /home/root/e3da/pkauth/conf/pkauth.conf; chmod 0644 /home/root/e3da/pkauth/conf/pkauth.conf"
```
...
```bash
sbin/pkauth-grant-admin e3da-srv1.e3da.wx cto
sbin/pkauth-grant-admin e3da-srv1.e3da.wx cto 6

# Use a local operator key directory for testing
sbin/pkauth-grant-admin e3da-srv1.e3da.wx cto 3 10022 admin@e3da.local /home/yarui/.ssh/keys

# Use another SSH port for a specific host
sbin/pkauth-grant-admin e3da-srv2.e3da.wx cto 3 22
```

The fourth argument is the SSH port; it defaults to `10022`. The fifth argument is the signing identity; it defaults to `admin@e3da.local`. The sixth argument is the local SSH key directory; it defaults to `/home/root/.ssh/keys`. Host, target user, grant duration, port, identity, and local key directory are command-line inputs. Token storage, logging, verifier location, manager GID, namespace, and signing-key relationship are fixed grant-script values and are not environment overrides.
...
The command is shell-independent for recipients. Users may run it from Bash, tcsh, or another shell:
...
The grant script itself requires Bash, OpenSSH `ssh-keygen`, GNU `date`, and `awk`. The signed expiration is a UTC-independent Unix epoch in milliseconds, and the verifier compares it with `Date.now()`. Grant output and audit log timestamps use the local timezone of the workstation or target host, respectively. Keep clocks synchronized with NTP. SSH `from=` restrictions remain enforced by OpenSSH and do not affect signature verification; the signing public key is trusted separately. Grant, rejection, and publication errors are logged to `/var/log/e3da/pkauth.log` on the target.
